...
\begin{figure}
	...
	\hspace*{0.1\textwidth}%
	% 
	\subfloat[Observers at rest relative to each other in a frame moving relative to them]{
...
\begin{figure}
	...
	\subfloat[Observers moving with velocities $v = -0.3 c, v' = 0.3 c$]{
		\includegraphics[width=0.4\textwidth]{pictures/synchronization_moving_observers_a.pdf}
	}
	%
	\hspace*{0.1\textwidth}
	%
	\subfloat[Observers moving with velocities $v = -0.1 c, v' = 0.4 c$]{
...
\begin{figure}
	...
	\subfloat[Observers at rest relative to each other in their rest frame]{\includegraphics[width=0.3\textwidth]{pictures/length_contraction_resting_observers_a.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat[Observers at rest relative to each other in a frame moving relative to them]{\includegraphics[width=0.38\textwidth]{pictures/length_contraction_resting_observers_b.pdf}}

	\subfloat[Observers moving with velocities $v = -0.3 c, v' = 0.3 c$.]{\includegraphics[width=0.36\textwidth]{pictures/length_contraction_moving_observers_a.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat[Observers moving with velocities $v = -0.1 c, v' = 0.4 c$.]{\includegraphics[width=0.38\textwidth]{pictures/length_contraction_moving_observers_b.pdf}}
	...
	% We indicate this rod length at one specific instant in time using orange color; solid orange lines represent the length of $\mathcal{L}, \mathcal{L}'$ as observed by $\mathcal{O}$, while dotted orange lines show the length of $\mathcal{L}, \mathcal{L}'$ as observed by $\mathcal{O}'$.\\
	...
	% \todo{account for change: length of $\mathcal{L}$ is solid orange now, while length of $\mathcal{L}'$ is teal, dotted; no matter which observer is making observation of lengths; feels like better solution}
...
Now, again, if you ask $\mathcal{O}'$ what $\mathcal{L}'$ is when $\mathcal{C}'$ shows $\tau'$ (which $\mathcal{O}$ would say is measurement that happens simultaneous to his measurement at time $t$), then the answer is still $l'$; but $\mathcal{O}'$ sees $\mathcal{L}$ shortened, i.e.~sees $l < l'$ -- length contraction is also mutual between observers. Moreover, since both observers always see rest length of their own rod, the length $l$ seen by $\mathcal{O}$ is the same as the $l'$ seen by $\mathcal{O}'$. Inequalities in inferred lengths only enter when observers make statements about the rods of other observers; this is related to, and perhaps even caused by (to some degree/in some sense) \todo{this previous sentence needs improvement}, the desynchronization of observers and thus $t \neq \tau'$ (just as simultaneity does not imply $t = \tau'$ and likewise, does not imply $l = l'$).


(I do acknowledge that visualizations such as Fig.~\ref{fig:length_contraction} can create the impression that this is somehow caused by our standard of simultaneity. But, once again, length contraction is not an effect caused by a bad standard of simultaneity; in fact, we use a relativistically correct/accurate standard.% -- and what it reveals is another relativistic effect.%
All we see here are consequences and manifestations of relativity itself.%
)



% -> it is all about where the ends of rods carried by $\mathcal{O}, \mathcal{O}'$ are at a given instant in time (note that the rods of course have the same rest length; i.e.~at $t = 0$ they have same length, if we assume observers are at rest with respect to each other there and start moving only afterwards; I guess we can claim them having same length is verified by referee in $t = 0$); in plot, we see the situation where the right ends of measuring rods carried by $\mathcal{O}, \mathcal{O}'$ are in same position at time where clock of $\mathcal{O}$ shows $t$; from the perspective (i.e.~frame) of $\mathcal{O}$, the left end of his own rod is at position where his clock shows $t$; the left end of the rod of $\mathcal{O}'$, however, is at the point $I$, as we verify by drawing light sent from $\mathcal{O}$ that meets this left end simultaneously to $t$
% -> we can think of measuring rods as lying parallel to the line of simultaneity of the respective observer


% -> the length of the rod $l'$, as seen by $\mathcal{O}$, can be determined by intersections of his line of simultaneity with the rod; length of his own rod is defined in the same manner, as intersection of world line of left and right rod end with \emph{the same} line of simultaneity; in particular, both rods are always parallel to line of simultaneity; (we choose plot so that right ends of both rods meet each other in event at time $\tau'$ on the clock of $\mathcal{O}'$, which means the rods of both observers are overlapping in the solid orange line); note that an absolute notion of \enquote{length of a rod} now becomes obsolete, we can only ever refer to length seen by an observer (after time standards became relative, length standards are now relative, too); this is evident in plot as well, the dotted orange line is length that $\mathcal{O}'$ measures for his own rod at $\tau'$ (on his clock) -- and this is clearly $l$, the same length that $\mathcal{O}$ measures for this rod at $t$ (and $\tau$, and any other time); and the mutuality continues, if we look at the length $\mathcal{O}'$ measures for the rod of $\mathcal{O}$ at time $\tau$, then the answer is $l'$ as well -- length contraction is mutual, just as we saw for time dilation
% -> in other words, if we ask for the length of their own rods at some time their clock shows (like $\tau = \tau'$), then both observers give the same answer; similarly, they give the same answer for what length the rod of the other observer has at this time; but when comparing the lengths they infer for the same rod, then the answers differ
% -> \todo{NO! This is just effect of one of the plots! where both of them move with same velocity relative to the rest frame} -> ah, I think this statement is true; but is not visible in all plots because rest observer in the respective plot has different relative velocities with respect to $\mathcal{O}, \mathcal{O}'$; if we were to analyze situation from rest frames of $\mathcal{O}, \mathcal{O}'$, then we would make observations described in paragraph above (I think)
...
% Let $A$ be the triangle spanned by the following three points: coordinate origin $A_0$, the point $A_\tau$ on $\mathcal{O}$ where $\mathcal{C}$ shows $\tau$, and the the point of intersection between the line of simultaneity ; similarly, let $B$ be the triangle spanned by the following three points: .
...
% -> since left end of $\mathcal{L}'$ is in point $A_\tau$ when right ends of rod meet (i.e.~when left end of $\mathcal{L}$ is in $B_t$)

% -> wie wir Faktor rauskriegen (aus Fig.~\ref{fig:length_contraction}): das Dreieck von Usprung zu tau und dann entlang der Gleichzeitigkeitslinie von O ist ähnlich zu dem von Ursprung zu t und dann entlang der Gleichzeitigkeitslinie; das heißt, das Verhältnis zwischen tau=tau', t und l', l ist gleich, sodass l = 1/gamma l' \todo{right? maybe draw extra plot for this}
...
\begin{figure}
	...
	\subfloat{\includegraphics[height=0.25\textheight, page=1]{pictures/length_contraction_similar_triangles.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat{\includegraphics[height=0.25\textheight, page=2]{pictures/length_contraction_similar_triangles.pdf}}
...
Evidently, moving rods are contracted by the exact same amount that moving clocks are dilated by. If you think this is a curious finding, here is some subtle foreshadowing: The same pattern that will prevail throughout relativity, $\gamma$ is all over the place. (Note that this is not meant to imply that $\gamma$ shows up so frequently \emph{because} it shows up here. I think this point is debatable, as one can also make the point that the appearance of $\gamma$ here comes from its appearance in the Lorentz transform that we derive later. It seems almost like a \enquote{chicken or egg} type question, and I have no definite answer for it.)% (Its occurrence in many different parts of the theory is a consequence of it appearing in the Lorentz transformation. \todo{well, but actually Lorentz is just transformation that properly incorporates time dilation+length contraction. So can we maybe take viewpoint that they are what causes this frequent appearance? -> not sure... I think Lorentz trafo is more general, and time dilation+length contraction are consequences of it})


-> \todo{note that phrase \enquote{moving rulers are shorter} is similar to \enquote{moving clocks run slowly}; is somewhat accurate, but can be misleading because the rulers do not actually change in size; but distances measured/perceived by moving observers are shorter than results in/from rest frame}
...
\begin{ex}[Complementarity of Time Dilation And Length Contraction]
	muon example (standard one, but see, e.g., 14.3 in Thornton)

	Setup: muons are created in the upper atmosphere \todo{stratosphere?} at heights of $h \approx 2000 \unit{\meter}$ move at $v = 0.98 c$ and have an average lifetime of $t_l = 1.52 \unit{\micro \second}$ -- yet they still arrive on Earth. This means they cover the full distance $h$, which would nominally/classically take them $t_t = \frac{h}{v} \approx 6.8 \unit{\micro \meter}$ -- much longer than $t_l$. Indeed, experimental observations show an excess amount of muons survive, compared to the classical expectation. How can this be explained?
	...
	% stationary observer on Earth: muon covers full distance from where it is created to ground, but due to speed it moves at it experiences time dilation and is thus actually able to cover this distance
	...
	% muon observer: on clock in this frame, the time interval between point of creation and point of decaying is measured to be usual lifetime of muon; but from muon frame, Earth distances are contracted; so what Earth observer reports as $2000 \unit{\meter}$ long distance corresponds to only $1/\gamma$ of the muon's own $2000 \unit{\meter}$ long measuring rod -- i.e.~from the muon perspective, much less distance needs to be covered, so it survives this in span of a single lifetime
	...
	% -> important takeaway: time dilation and length contraction are, in some sense, equivalent effects; so they are not distinct in the sense that they must be added up for observer moving close to speed of light (when examined from Earth frame, we do not have to account for lengthened lifetime \emph{and} shorter rulers, these effects do not occur together in that sense, but rather ensure that different observers make complementary conclusions -> and also that questions that we ask differently yield equivalent answers; in some sense, first viewpoint/question looks at how much time the particle needs to travel between two fixed points in space, so that time dilation plays role; the second viewpoint/question looks at how much distance each observer can cover in a given time interval, so length contraction plays a role)
	% -> also wrong; length is \emph{also} contracted for the moving observer; but this is just not interesting here, this effect does not matter; time dilation does matter, since this explains why particle can seemingly live longer than it should (its internal perception of time is dilated), but \enquote{what length does the particle record} is simply not an interesting question
	% -> so the effects \emph{certainly} occur together; in fact, this ensures some kind of consistency; from the perspective of $\mathcal{O}$, let the speed of the muons be $v$; then primed time is slowed down, so less goes by, but primed ruler is also contracted, so length traveled by particle is also smaller; so their quotient, which represents particle speed, still yields $v$ \todo{right?}; so at least, observations made from a single frame, but using different devices are somewhat internally consistent
	...
	% -> how can they be equivalent if times are dilated, while rulers are contracted? Well, time dilation = less time goes by on clocks carried by moving observers = they perceive the same time interval to be shorter, and length contraction = rulers are shortened = they measure higher the same length to be larger -> both times, something gets smaller, but implications are very different (because once, it is the actual thing we measure that becomes smaller, while in other case it is the measuring device that gets shorter \todo{is that correct?})

	% -> nah, do not say it this way. I mean I get the point and idea (and it might not even be wrong), but I am not sure if this point can be made clear enough to not be confusing
	...
	% -> yeah, we should definitely refer to rods of length corresponding to distance we want to measure; much better than thinking about rods with unit lengths, which are then stacked (is equivalent, but one more step in process; tends to lead to more confusion)
\end{ex}
...
\begin{figure}
	...
	\subfloat[Receiver rest frame]{\includegraphics[width=0.4\textwidth,page=1]{pictures/aberration.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat[Source rest frame]{\includegraphics[width=0.4\textwidth,page=2]{pictures/aberration.pdf}}
...
\todo{I think this might not be 100\% correct... We must boost by $-v$}
...
\todo{maybe show different version of calculation, example 12.6 from Griffiths?}
