...
More specifically, Eq.~\eqref{eq:doppler_factor} encapsulates the \Def[Doppler effect!longitudinal]{longitudinal Doppler effect}, where the relative movement of $\mathcal{O}, \mathcal{O}'$ is along the line of sight $\vec{N}$ between them.
%More generally, we have to take component of $\vec{v}$ along $\vec{N}$, i.e.~project onto this vector. In formulas, that corresponds to replacing
%\begin{equation}
%	v = \norm{v} \mapsto v \cos \theta, \; \text{ where } \theta = \angle(\vec{v}, \vec{N}) = \frac{\langle \vec{v}, \vec{N} \rangle}{\norm{v} \norm{N}} \, .
%\end{equation}
%\todo{but not in sqrt! So change this -> or maybe also there? Yielding the modified formulas from Dragon}
...
% but we omit this here because it arises naturally as a coordinate transformation, too, and is thus are covered in next section.% (strictly speaking, we have looked at boosts here, right? And things like rotations are then part of more general Lorentz transformation)
\\
...
\end{equation}
Inserting this result into Eq.~\eqref{eq:lorentz_time_ansatz} and Eq.~\eqref{eq:lorentz_spatial_ansatz} yields the \Def{Lorentz transformation}
