...
it so that Eq.~\eqref{eq:synchr_explicit} holds.
...
for a ruler of length $l$ carried by $\mathcal{O}$.
		...
		\subsection{Doppler Shift \& Aberration}\label{subsec:doppler}
...
from Eq.\eqref{eq:doppler_derivation} we have already seen what \Def{Doppler factor} is, namely
\begin{equation}
	\eqbox{\kappa = \sqrt{\frac{1 + v / c}{1 - v / c}} = \frac{1 + v / c}{\sqrt{1 - v^2 / c^2}}}
\end{equation}
this is classical Doppler, but with the same factor that we had seen for time and lengths already

\todo{show cool form from \cite{Tonkinson_2009}, where relation with classical Doppler is explored}
...
The last step is determining the factor $\gamma$, which can be done by inserting Eq.~\eqref{eq:lorentz_time_ansatz} into Eq.~\eqref{eq:lorentz_spatial_ansatz}
\begin{align*}
