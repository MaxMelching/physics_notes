...
\pgfkeys{
	/spacetimediagramnew/.is family, /spacetimediagramnew,
...
\newcommand{\spacetimediagramnew}[2][]{%
	\pgfkeys{/spacetimediagramnew, default, #1}
...
\pgfkeys{
	/addobservernew/.is family, /addobservernew,
...
% Mandatory argument: grid size, relative velocity
% Optional arguments: stepsize (sets grid scale), xlabel, ylabel
\newcommand{\addobservernew}[3][]{%
	\pgfkeys{/addobservernew, default, #1}
	...
	%\newboolean{\gridTest}
	%\setboolean{\gridTest}{\observerGrid}
	%\ifthenelse{\gridTest = true}{%
	\ifthenelse{\equal{\observerGrid}{true}}{%
	% Draw the vertical transformed grid lines
    \foreach \x in {-#2,...,#2}
        \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(\observerxpos,\observerypos)}, blue, very thin] (\x * \observerStepsize, -#2 * \observerStepsize) -- (\x * \observerStepsize, #2 * \observerStepsize);%
    % Draw the horizontal transformed grid lines
    \foreach \y in {-#2,...,#2}
        \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(\observerxpos,\observerypos)}, blue, very thin] (-#2 * \observerStepsize,\y * \observerStepsize) -- (#2 * \observerStepsize, \y * \observerStepsize);%
	}{}
...
\newcommand{\spacetimediagram}[2]{%
	...
	% Draw light cone
	\draw[black!10!yellow, thick] (-#2*#1, -#2*#1) -- (#2*#1, #2*#1);
	\draw[black!10!yellow, thick] (-#2*#1, #2*#1) -- (#2*#1, -#2*#1);
}

% Arguments: scale of grid, cone size, x-position of event, ct-position of event
% Arguments: x-position of event, ct-position of event, cone size
\newcommand{\lightcone}[4][1]{
	...
	%\draw[yellow, fill, fill opacity=0.42] (#1 - #3, #2 + #3) -- (#1, #2) -- (#1 + #3, #2 + #3);
	%\draw[yellow, fill, fill opacity=0.42] (#1 - #3, #2 - #3) -- (#1, #2) -- (#1 + #3, #2 - #3);

	\draw[yellow, fill, fill opacity=0.42] (#3*#1-#2*#1, #4*#1+#2*#1) -- (#3, #4) -- (#3*#1+#2*#1, #4*#1+#2*#1);
	\draw[yellow, fill, fill opacity=0.42] (#3*#1-#2*#1, #4*#1-#2*#1) -- (#3, #4) -- (#3*#1+#2*#1, #4*#1-#2*#1);
}
...
\newcommand{\addshiftedobserver}[6]{%
    % Evaluate the Lorentz transformation
    %\FPeval{\calcgamma}{1/((1-(#3)^2)^.5)}
    \FPeval{\calcgamma}{1/((1-((#3)*(#3)))^.5)} % More robust, allows negative v
    \FPeval{\calcbetagamma}{\calcgamma*#3}
	...
	% Draw the x' and ct' axes
	\draw[->,thick,cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (-#2*#1-#1, 0) -- (#2*#1+#1, 0);
    \draw[->,thick,cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (0, -#2*#1-#1) -- (0, #2*#1+#1);

	% Check if grid shall be drawn
	\ifthenelse{\isempty{#4}}{}{%
	% Draw the vertical transformed grid lines
    \foreach \x in {-#2,...,#2}
        \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue,very thin] (\x*#1,-#2*#1) -- (\x*#1,#2*#1);%
    % Draw the horizontal transformed grid lines
    \foreach \y in {-#2,...,#2}
        \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue,very thin] (-#2*#1,\y*#1) -- (#2*#1,\y*#1);%
	}

	% Draw the x' and ct' axes labels
    \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (#2*#1+#1+0.2,0) node {$x^\prime$};
    \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (0,#2*#1+#1+0.2) node {$ct^\prime$};
}
...
\newcommand{\addobservernewversionone}[6][]{%
    % Evaluate the Lorentz transformation
    %\FPeval{\calcgamma}{1/((1-(#3)^2)^.5)}
    \FPeval{\calcgamma}{1/((1-((#4)*(#4)))^.5)} % More robust, allows negative v
    \FPeval{\calcbetagamma}{\calcgamma*#4}

	% Draw the x' and ct' axes
	\draw[->,thick,cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (-#3*#2-#2, 0) -- (#3*#2+#2, 0);
    \draw[->,thick,cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (0, -#3*#2-#2) -- (0, #3*#2+#2);

	% Check if grid shall be drawn
	\ifthenelse{\isempty{#1}}{%
	% Draw the vertical transformed grid lines
    \foreach \x in {-#3,...,#3}
        \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue,very thin] (\x*#2,-#3*#2) -- (\x*#2,#3*#2);%
    % Draw the horizontal transformed grid lines
    \foreach \y in {-#3,...,#3}
        \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue,very thin] (-#3*#2,\y*#2) -- (#3*#2,\y*#2);%
	}{}

	% Draw the x' and ct' axes labels
    \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (#3*#2+#2+0.2,0) node {$x^\prime$};
    \draw[cm={\calcgamma,\calcbetagamma,\calcbetagamma,\calcgamma,(#5,#6)},blue] (0,#3*#2+#2+0.2) node {$ct^\prime$};
}