...
\caption{Tabular representation of the sample space for double roll of a four-faced die.}
\fi
...
\begin{prop}[(Kolmogorov) Axioms of Probability Theory]
\begin{enumerate}[(i)]
...
% counting number of outcomes in event is special case of splitting event into single outcomes such that it is union of them and then using total probability to deduce that probability of event = sum of probabilities of outcomes
...
\end{equation*}
$P(R | A)$ is just the detection efficiency (known), $P(A)$ is our guess/belief how often an airplane is present when looking up in the first place (estimated to $5\%$) and% $P(R)$ can be computed using the total probability rule:
\begin{equation*}
...
% example with numbers: each $A, B$ have probability $1 / 4$, with $1 / 16$ of that being in $A \cap B$; then, $P(A) P(B) = 1 / 16 = P(A \cap B)$ (independence), but since $A \cap B \cap C = \emptyset$ they are not independent when conditioned on $C$ (which can hold the remaining $1 / 2$ of probability, doesn't really matter) since $P(A) P(B) P(C) = 1 / 4 \cdot 1 / 4 \cdot 1 / 2 = 1 / 32 \neq 0 = P(A \cap B \cap C)$
...
\footnotetext{As a sidenote: Bayes' theorem allows us make this statement more quantitative ($A := \{90 \, H \, \text{ in } \, 100 \, \text{ flips}\}$), yielding $P(C_1 | A) = \frac{P(C_1) P(A | C_1)}{P(C_1) P(A | C_1) + P(C_2) P(A | C_2)} = \frac{0.5 \cdot 0.9^{100}}{0.5 \cdot 0.9^{100} + 0.5 \cdot 0.1^{100}} = 0.9999\dots$, which is indeed \emph{much} more likely than the probability of flipping the second coin, $P(C_2 | A) = 1 - P(C_1 | A) = 3.76 \cdot 10^{-96}$.}
...
\begin{tikzpicture}[grow=right,level 1/.style={level distance=5\baselineskip}]
\node[circle, fill]{}
    child{node[circle, fill]{}%{$100\$$} % Only confuses
	    child{node{$300\$$}
	    edge from parent[below]node{$0.5$}
		}
	    child{node{$100\$$}
...
\begin{tikzpicture}[grow=right,level 1/.style={level distance=5\baselineskip}]
\node[circle, fill]{}
    child{node[circle, fill]{}%{$200\$$} % Only confuses
	    child{node{$300\$$}
	    edge from parent[below]node{$0.8$}
		}
	    child{node{$200\$$}
...
\begin{itemize}
\item \Def{Uniform distribution}: the simplest distribution one can think of is a constant one, $f_X(x) = k$ on some interval $[a, b]$. For an explicit expression, we can calculate $k$:
...
\footnotetext{Note that we do not demand $Y = y$ anymore because $P(Y = y) = 0$ (does not make sense to do that).}
\addtocounter{footnote}{1}
\footnotetext{$E[X]= E_{X, Y}[X]$ specifically reduces to $E_X[X]$, i.e.~the one using $f_X$, because of \eqref{eq:marginals}.}
...
This is all we need because these quantities allow the computation of the normalization constant (sometimes called \Def{evidence}) by expressing it as a marginal probability:
\begin{equation*}
...
\includegraphics[width=\textwidth]{pictures/posteriors_no_max_v2.pdf}
...
%second example: given velocity-distribution $v \sim \mathcal{U}(30, 60)$ (uniform in this interval), we want distribution of travel-time $t = \frac{s}{v}$ that we need for certain distance $d$
\end{ex}
\footnotetext{Remembering that dividing by a negative number changes the \enquote{direction} of an inequality.}
...
\end{equation}
\end{prop}
The average conditional expectation matches the \enquote{regular} expectation. %Basically, this is a direct corollary of the total expectation theorem. -> is just calculation, not more to it
...
\end{equation}
where we abbreviate $\mu = E[X]$ and replaced $c \rightarrow c^2$ for convenience. The statement becomes more obvious if we set $c = k \sigma_X$ (in this form, it is called the \Def{Chebyshev Inequality}\footnote{Note that $(X - \mu)^2$ is a positive RV for any $X$, not only $X > 0$. Thus, \eqref{eq:cor_markov} and \eqref{eq:chebyshev} hold for arbitrary $X$.}):
...
A sequence $\qty{X_n}_{n \in \mathbb{N}}$ of RVs \Def[convergence in probability]{converges in probability} to a CRV $X$ if
\begin{equation}
...
% cool example: we can estimate $p$ for Bernoulli experiment by performing it repeatedly and then use that expectation of binomial is $N p$, i.e.~by computing sample mean (dividing binomial, which is just sum of the RVs, by $N$) we can compute $p$; moreover, this is way to derive expectation, actually (nonetheless, it also gives us way to estimate $p$, both is true)
