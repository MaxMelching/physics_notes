...
\begin{equation}
\lambda_{ijk} = \Gamma_{ij}^k \, .% = \tilde{e}^k\qty(\nabla_{\pdv{x_i}} e_j) \, .\footnote{Test}
\end{equation}
...
\end{equation}
-> sollte hier nicht auch $\lambda_{ijk}$ statt $\Gamma_{ij}^k$ stehen?
...
\begin{align*}
\pdv{x_i} g_{jk} + \pdv{x_j} g_{ik} &= \pdv{x_k} g_{ij} + 2 g\qty(\sum_l \Gamma_{ij}^l \pdv{x_l}, \pdv{x_k})% = \pdv{x_k} g_{ij} + 2 \sum_l \Gamma_{ij}^l g_{lk}
\\
...
Mittels der Variablensubstitution $\dv{\tilde{\gamma}_j}{t} =: u_j$ lässt sich diese DGL 2.~Ordnung in eine DGL 1.~Ordnung (für die $u_j$, also auf $T \mathbb{R}^n$) umschreiben, die \Def{Euler-Lagrange-Gleichung} (sehr relevant in der Physik im Kontext von Phasenräumen). Das hat die wichtige Folgerung, dass Geodäten durch Anfangspunkt und -richtung bereits eindeutig bestimmt sind (gilt bei Geraden ganz offensichtlich, aber somit allgemein).\\
...
%-> ok, wie ich es verstehe interessiert uns die explizite Wirkung auf Schnitte gar nicht, eher wie $R$ \enquote{aussieht} (und insbesondere ob null oder nicht) -> wir betrachten es hier nicht zumindest
...
\end{equation}
komponentenweise die \Def[Zusammenhangs-1-Form]{Zusammenhangs-1-Form $A$ (zu $\nabla$)} mit $A \in \Gamma(M; T^*M \otimes E^* \otimes E)$ zu definieren. $e_j \in E$ bezeichnet dabei wie üblich einen Basisvektor.
