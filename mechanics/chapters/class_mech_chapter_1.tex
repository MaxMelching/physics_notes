...
\todo{take over some of the things from Thornton 2.4 (he has a recipe for solving equation of motion)}
...
-> if also function of velocity (to whatever power, if even at all), they are called \Def{retarding forces} -> see Thornton p.~58ff
...
\end{equation}
Since this is the work that has to be done to get the particle in motion, this energy $T$ is termed \Def[energy!kinetic]{kinetic energy} and sometimes denoted by $E_\mathrm{kin}$. (Notice that the explicit value of $T$ depends on the chosen reference frame since it involves the velocity $v$. Similar to the frame dependencies encountered before, this is fine, because the physics inferred from it is still the same.) The work done on a path against the inertial force can then be written as
\begin{equation}
	\eqbox{
...
\todo{check if these statements are all is actually true or a little too strong sometimes}
...
Energy (work) is also convenient because it allows a useful classification of forces. Some of them have a \Def{potential} $V = V(\vec{x}, t)$ (though it often has no time-dependence; in some cases, $V$ also depends on $\dot{\vec{x}}$) that can be used to express them as
\begin{equation}\label{eq:conservative_force}
	\eqbox{
		\vec{F} = - \grad V = - \pdv{\vec{x}} V
	} \, .
\end{equation}
This type of forces is called \Def[force!conservative]{conservative}, while non-conservative forces are also called \Def[force!dissipative]{dissipative}. $V$ has dimensions of energy and is also called \Def[energy!potential]{potential energy}. Conservative forces play a special role in how they interact with the energy of a system. By some (quite beautiful) mathematical theorem, the property we noted as a definition of conservative forces implies
...
\begin{ex}[Finding The Right Sign]
...
\begin{figure}
	...
	\subfloat[Free harmonic oscillator (initial condition indicated as dark red point)]{\includegraphics[width=0.4\textwidth,page=1]{pictures/dynamics_phaseportrait/dynamics_phaseportrait.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat[Two free harmonic oscillators with different periods]{\includegraphics[width=0.4\textwidth,page=2]{pictures/dynamics_phaseportrait/dynamics_phaseportrait.pdf}}

	\subfloat[Damped harmonic oscillator]{\includegraphics[width=0.4\textwidth,page=3]{pictures/dynamics_phaseportrait/dynamics_phaseportrait.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat[Pendulum]{\includegraphics[width=0.5\textwidth,page=4]{pictures/dynamics_phaseportrait/dynamics_phaseportrait.pdf}}
...
\end{equation}
\todo{mention how it can also be written as linear transformation using a skew-symmetric matrix? Cf.~\url{https://en.wikipedia.org/wiki/Angular_velocity_tensor}}
...
% -> idea: define in differential manner as (something), i.e.~$d\vec{L} = m r^2 d\phi$ -> hmm idea was that $r d\phi$ is infinitesimal rotation step, but don't think this helps us at the moment...
...
\begin{align}
	...
	%\underset{\text{point particle}}{=} \qty(\dv{m}{t} r^2 + 2 m r \dv{r}{t}) \vec{\omega} + I \dv{\vec{\omega}}{t}
\end{align}
...
-> main takeaway: one must exert a moment about the rotation axis to change angular momentum (this is what torque is \todo{right?})
...
Lastly, we want to derive an expression for the kinetic energy involved in rotational motion, i.e.~an expression for \Def{rotational energy}. Usually, we compute the work done by a force starting from $dW = \vec{F} \cdot d\vec{x}$. But now we want to express this in terms of rotational quantities, where it becomes
\begin{equation}
...
\todo{look at this bit here. think is largely the same, but make sure of that} \url{https://en.wikipedia.org/wiki/Work_(physics)#Torque_and_rotation}
...
\todo{this is actually not true! Kinetic energy is intrinsically dependent on frame we select, even for inertial ones; but this is ok, physically relevant statements like conservation of energy have no such dependence (in particular, inferred laws also do not have this)}
...
\end{equation}
We call such systems as a whole \Def{conservative} (we mark this as a new term here because we only knew conservative forces before).
...
% this might seem puzzling since $V_{kj} = V_{jk}$, while $\grad_k V_{jk} = - \grad_j V_{jk}$, but this is very natural if, for example, $V_{jk} \propto r^m = \norm{\vec{r}_j - \vec{r}_k}^m$; then, while exchanging the order of $\vec{r}_j, \vec{r}_k$ may not change the value of $V_{jk}$, but it does change the associated function $V_j(\vec{r}) \propto \norm{\vec{r} - \vec{r}_j}$ that fulfills $V_{jk} = V_j(\vec{r}_k)$ because the direction of change does not remain constant when exchanging the order (basically, just ask yourself this: what happens when I change $\vec{r}_j$ a little bit vs.~what happens when I change $\vec{r}_k$ a little bit; there is a sign difference in them, which translates to sign difference in the gradient) \todo{too verbose...}
...
\begin{figure}
	...
	\subfloat[$\frac{m_1}{M} = 0.25, \frac{m_2}{M} = 0.75$]{\includegraphics[width=0.25\textwidth,page=1]{pictures/central_force_orbits.pdf}}%
	\hspace{0.1\textwidth}%
	\subfloat[$\frac{m_1}{M} = 0.5, \frac{m_2}{M} = 0.5$]{\includegraphics[width=0.25\textwidth,page=2]{pictures/central_force_orbits.pdf}}%
	\hspace{0.1\textwidth}%
	\subfloat[$\frac{m_1}{M} = 0.001, \frac{m_2}{M} = 0.999$]{\includegraphics[width=0.25\textwidth,page=3]{pictures/central_force_orbits.pdf}}
...
Let us look at numbers first and then get some intuition on the whole situation. Since $u_2 = 0$, % and accounting for the fact that the COM velocity is conserved even after the collision,
% \begin{equation}
% 	m_1 u_1 = m_1 u_1 + m_2 u_2 = M V = m_1 v_1 + m_2 v_2 \, .
% \end{equation}
% -> $m_1 u'_1 + m_2 u'_2 = m_1 v'_1 + m_2 v'_2$ also
% This is just a consequence of conservation of momentum. Therefore,
...
% 
% -> think about it like this: we draw circle around $\vec{V}$, whose boundary touches/goes through $\vec{v}_1$; CMS laws dictate that $\vec{v}'_1 = \vec{v}_1 - \vec{V}$, so we only have unique identification if $V < v'_1$; in Figure, $v'_{1, b} = v'_{1, f} = v'_1$, but direction is different
% 
...
\begin{figure}
	...
	% \subfloat[Initial State]{\includegraphics[width=0.5\textwidth, page=1]{pictures/collision_states.pdf}}%
	% \hspace*{0.1\textwidth}%
	% \subfloat[Final State]{\includegraphics[width=0.4\textwidth, page=2]{pictures/collision_states.pdf}}
	\subfloat[Initial State]{\includegraphics[width=0.7\textwidth, page=1]{pictures/collision_states.pdf}}
	
	\subfloat[Final State]{\includegraphics[width=0.6\textwidth, page=2]{pictures/collision_states.pdf}}

	\caption{States involved in an elastic collision}
	\label{fig:collision_states}
\end{figure}
...
\begin{figure}
	...
	\subfloat[$m_1 < m_2$]{\includegraphics[width=0.4\textwidth, page=1]{pictures/collision_final_state_m1.pdf}}%
	\hspace*{0.1\textwidth}%
	\subfloat[$m_1 > m_2$]{\includegraphics[width=0.4\textwidth, page=2]{pictures/collision_final_state_m1.pdf}}

	\caption{Different solutions for $\vec{v}'_1$, depending on the ratio of $m_1, m_2$.}
	\label{fig:collision_m1_cases}
\end{figure}
...
\end{equation}
i.e.~to just add the \Def{$Q$-value} (a very creative name). $Q = 0$ corresponds to elastic collisions; $Q > 0$ corresponds to \Def{exergic} collisions, where energy is gained during the collision; $Q < 0$ corresponds to \Def{endoergic} collisions, where energy is lost during the collision.
...
-> measure of inelasticity: \Def{coefficient of restitution}
\begin{equation}
	\eqbox{
...
During any type of collision, the forces exerted by the colliding bodies typically act over a short period of time. The type of forces involved there are called \Def{impulsive forces}.
