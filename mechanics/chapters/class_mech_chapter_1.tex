...
\todo{reconsider if we want to split up evolution of origin and orientation of axes. I find that a little confusing...; then we'd have $\vec{e}'_i = \delta \vec{x}_0 + R \vec{e}_i$ instead of $\vec{e}'_i = R \vec{e}_i$, as it is now} -> then we would also have $\vec{x}$ as a purely abstract, geometric quantity, which does not require any coordinate system yet -- it CAN be expanded in the different coordinates, upon choice of origin and axes orientation, but is just generally used to denote the position (this does not need coordinates; also emphasized by Thorne+Blandford I think)
\begin{itemize}
...
\todo{I think it makes sense to remove shift here, otherwise things look strange}
...
\todo{check if the math is solid here. not sure if these terms make sense to me}
