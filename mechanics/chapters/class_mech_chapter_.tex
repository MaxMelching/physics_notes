...
-> fields are cancelled because there are so many free charges -> and only then we have equilibrium (also seems to happen instantaneously, for all practical matters)
...
\todo{treat 4.4.4? \enquote{Forces on Dielectrics}}
...
\end{equation}
(Reminder: $\vec{n}$ is the unit normal vector to the surface.)
