...
-> \todo{$d\mathcal{C}'$ or $d\mathcal{V}'$?}
...
\todo{do all the interesting things with dipole energy + force, torque}
...
\end{align}
\todo{I only had this in my notes with $\vec{r}_0 = 0$, but not sure why this is required, so I added arbitrary point; anyway, I do not understand all steps}
\todo{maybe we can argue that $\int \vec{j}(\vec{r}') \cross \vec{r}_0 \, d\mathcal{V}'$ must vanish, that would be first step on the way; and seems kind of reasonable actually, is something like total circulation or so}
\todo{ahhh, progress: as Griffiths notes before Eq.~(6.2), the total displacement around a closed loop always vanishes, which should imply $\int \vec{j}(\vec{r}') \, d\mathcal{V}' = 0$; helps with $\cross \vec{B}(\vec{r}_0)$ term and also $\cross \vec{r}_0$ term}
