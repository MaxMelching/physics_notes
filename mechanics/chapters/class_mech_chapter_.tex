...
\todo{rather look at $\grad_{\vec{a}}$ at $\vec{a} = 0$? Then $\grad_{\vec{x}}$ follows directly, nice for field calculations below}
...
\end{equation}
where $S(C)$ is the area that is enclosed by $C$ (equivalently, we could replace $C \mapsto \partial S, S(C) \mapsto S$). This is the (integral form of the) first Maxwell equation of electrodynamics. To honor its discoverer, this equation is also called \Def{Faraday's law}. In differential form, it reads
