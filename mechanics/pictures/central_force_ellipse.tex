...
\begin{tikzpicture}
    ...
    \draw[mylabelarrow] (F1) -- (p) node[mylabel] {$p$};  % Semi-latus rectum
    ...
    \draw[COMbody] (relcoord) circle (0.07) node[COMbodylabel, above] {$\mu$};
    \draw[->, thick, COMbody] (F1) -- (relcoord) node[COMbodylabel, midway, fill=white,] {$\vec{r}$};
\end{tikzpicture}
