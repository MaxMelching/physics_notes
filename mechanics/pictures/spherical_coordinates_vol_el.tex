...
\begin{tikzpicture}[scale=2.8,tdplot_main_coords]
    ...
    % -- No +dr, otherwise we have to use $(r+dr)d\theta$ etc. Could neglect terms quadratic in $dr$ in the end, so same result, but unnecessarily complicated -> BS
    ...
    \draw[element lines] (Pmmm) -- (Pmmp) -- (Ppmp);  % Some contour in the back
