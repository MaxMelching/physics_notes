...
% Heavily inspired by Figs. 2.36, 2.37 in Griffiths Electrodynamics (4th edition)
...
\begin{tikzpicture}[scale=0.7]
    ...
    \draw[labelarrow] (7,-1.6) --++ (0,1.4) node[midway,right] {$\vec{E}_{\perp, \mathrm{below}}$};
    \draw[labelarrow] (7,1) --++ (0,1.4) node[midway,right] {$\vec{E}_{\perp, \text{above}}$};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.7]
    ...
    \draw[
        ...
        decoration={
            markings,
            mark=between positions 0.045 and 1 step 0.25 with \arrow{>}% V1
            % mark=at position 0.5 with \arrow{>}% V2
        },
    ] (\Rx,\Ry) ++ (0,0.12) ++ (1,-0.3) --++ (0,-0.12*2) --++ (2,0.7) --++ (0,0.12*2) --++ (-2,-0.7);
    ...
    ] (\Rx,\Ry) ++ (0,-0.12) ++ (1,-0.3) --++ (2,0.7) node[midway,below right=4]{$l$};
    

    % \draw[fill,myblue] (7.7,0.9) circle(0.07);
    % \draw[labelarrow] (7.7,0.6) --++ (-1,-0.35) node[midway,below right] {$\vec{E}_{\parallel, \mathrm{below}}$};
    % \draw[labelarrow] (7.7,1.2) --++ (-1,-0.35) node[midway,above left=1] {$\vec{E}_{\parallel, \text{above}}$};
    % -- Same ratio as the edge with (2,0.7), which is edge we are trying to be parallel to

    \draw[labelarrow] (7.7,0) --++ (-1,-0.35) node[midway,below right] {$\vec{E}_{\parallel, \mathrm{below}}$};
    \draw[labelarrow] (7.7,0.4) --++ (-1,-0.35) node[midway,above=2] {$\vec{E}_{\parallel, \text{above}}$};
\end{tikzpicture}
